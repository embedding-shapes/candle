#[cfg(feature = "cuda")]
#[test]
fn test_mmq_4096_2880() -> candle_core::Result<()> {
    use candle_core::{Device, Tensor, DType};

    let device = Device::new_cuda(0)?;
    let rows = 1;
    let out_dim = 2880;  // Hidden dimension (O projection output)
    let in_dim = 4096;   // Q dimension (O projection input)
    let nblocks = in_dim / 32;

    eprintln!("Testing MMQ kernel with O projection dimensions:");
