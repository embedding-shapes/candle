...
fn test_real_gpt_oss_weights_cpu_vs_cuda() -> Result<()> {
    ...
    let blocks_all = tensors.get("model.layers.0.mlp.experts.gate_up_proj_blocks")
        .expect("blocks tensor not found");
    let scales_all = tensors.get("model.layers.0.mlp.experts.gate_up_proj_scales")
    ...
    for i in 0..32 {
        ...
        if diff > 1e-6 {
            println!("  [{}] CPU: {:.6}, Expected: {:.6}, Diff: {:.6}", 
    ...
    for i in 0..32 {
        ...
        if diff > 1e-6 {
            if mismatch_count < 10 {  // Only print first 10 mismatches
                println!("  [{}] GPU: {:.6}, Expected: {:.6}, Diff: {:.6}", 
    ...
    if gpu_matches {
        ...
        println!("  ✗ GPU has {} mismatches out of 32 values!", mismatch_count);
    }
    
    // The test assertion
    assert!(cpu_matches, "CPU dequant should match expected values");
    assert!(gpu_matches, "GPU dequant should match expected values (BUG: this will fail)");
