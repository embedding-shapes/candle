...
fn t20_missing_mxfp4_pair_reports_clear_error() -> Result<()> {
    ...
    // Essentials
    tmap.insert("model.embed_tokens.weight".into(), bf16((cfg.vocab_size, cfg.hidden_size), &dev));
    tmap.insert("model.norm.weight".into(), bf16((1, cfg.hidden_size), &dev).reshape(cfg.hidden_size)?);
    let l0 = 0usize;
    let q_out = cfg.num_attention_heads * cfg.head_dim.unwrap();
    let kv_out = cfg.num_key_value_heads * cfg.head_dim.unwrap();
    tmap.insert(format!("model.layers.{l0}.self_attn.q_proj.weight"), bf16((q_out, cfg.hidden_size), &dev));
    tmap.insert(format!("model.layers.{l0}.self_attn.k_proj.weight"), bf16((kv_out, cfg.hidden_size), &dev));
    tmap.insert(format!("model.layers.{l0}.self_attn.v_proj.weight"), bf16((kv_out, cfg.hidden_size), &dev));
    tmap.insert(format!("model.layers.{l0}.self_attn.o_proj.weight"), bf16((cfg.hidden_size, q_out), &dev));
    tmap.insert(format!("model.layers.{l0}.self_attn.sinks"), bf16((1, cfg.num_attention_heads), &dev).reshape(cfg.num_attention_heads)?);
    tmap.insert(format!("model.layers.{l0}.mlp.router.weight"), bf16((cfg.num_local_experts, cfg.hidden_size), &dev));
    ...
    tmap.insert("lm_head.weight".into(), bf16((cfg.vocab_size, cfg.hidden_size), &dev));

    let vb = candle_nn::VarBuilder::from_tensors(tmap, DType::BF16, &dev);
    let err = GptOssModel::load(vb, &cfg).unwrap_err();
    let msg = format!("{err}");
    assert!(msg.contains("gate_up_proj") && msg.contains("missing") && msg.contains("scales"),
