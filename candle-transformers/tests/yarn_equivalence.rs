use candle::{Device, Result, Tensor, DType, D};
...
fn yarn_scaling_equivalence_softmax_vs_sincos() -> Result<()> {
    ...
    let s_a = (q_a.contiguous()?.matmul(&k_a.transpose(2, 3)?.contiguous()?)? * scale_a as f64)?; // (b,h,t,t)
    ...
    let s_b = (q_b.contiguous()?.matmul(&k_b.transpose(2, 3)?.contiguous()?)? * scale_b as f64)?;
    ...
    assert!(max_diff < 1e-4, "YARN equivalence broken: max_diff={}", max_diff);
    Ok(())
}
