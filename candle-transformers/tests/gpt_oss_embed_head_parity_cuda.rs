...
fn gpt_oss_embed_head_parity_cuda() -> Result<()> {
    ...
    let emb_cpu = vb_cpu.pp("model.embed_tokens").get((cfg.vocab_size, cfg.hidden_size), "weight")?;
    let lm_cpu = vb_cpu.pp("lm_head").get((cfg.vocab_size, cfg.hidden_size), "weight")?;
    let (er, ec) = emb_cpu.dims2()?;
    let (lr, lc) = lm_cpu.dims2()?;
    eprintln!(
        "embed_tokens.weight: dtype={:?} device={:?} shape=({}, {}) stride={:?}",
        emb_cpu.dtype(), emb_cpu.device(), er, ec, emb_cpu.stride()
    );
    eprintln!(
        "lm_head.weight:       dtype={:?} device={:?} shape=({}, {}) stride={:?}",
        lm_cpu.dtype(), lm_cpu.device(), lr, lc, lm_cpu.stride()
    );
    ...
    assert!(!same_storage, "embed and lm_head must be untied (distinct storages)");
    ...
    for &t in &sample_ids {
        ...
        for i in 0..ec {
            let ev = e_vec[i];
            let hv = h_vec[i];
            if !ev.is_finite() { e_all_finite = false; }
            if !hv.is_finite() { h_all_finite = false; }
            e_sum += ev * ev;
            h_sum += hv * hv;
        }
        let e_l2 = e_sum.sqrt();
        let h_l2 = h_sum.sqrt();
        eprintln!("t={} ||emb[t]||2={:.6} ||lm[t]||2={:.6} finite: emb={} lm={}", t, e_l2, h_l2, e_all_finite, h_all_finite);
    ...
    for &i in &idxs {
        ...
        eprintln!("x: dtype={:?} device={:?} shape={:?} stride={:?} i={}", x.dtype(), x.device(), x.dims(), x.stride(), i);
        ...
        assert!(linf <= 1e-6, "orientation mismatch at i={} (L_inf={})", i, linf);
    }
...
#[test]
fn gpt_oss_embed_head_parity_cuda_skipped() { eprintln!("skipped: build without 'cuda' feature"); }
