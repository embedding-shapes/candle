...
fn gpt_oss_lm_head_contract_cuda() -> Result<()> {
    ...
    if let Some(b) = &b_cpu {
        eprintln!(
            "lm_head.bias: dtype={:?} device={:?} shape={:?}",
            b.dtype(),
            b.device(),
            b.dims()
        );
    ...
    let b = match b_cpu {
        Some(bc) => Some(bc.to_device(&dev)?.to_dtype(DType::F32)?),
        None => None,
    };
    ...
    for &i in &idxs {
        ...
        l2_cc = l2_cc.sqrt();
        eprintln!(
            "i={} L_inf(candle,manual)={:.3e} L2={:.3e} L_inf(candle,column)={:.3e} L2={:.3e}",
            i, linf_cm, l2_cm, linf_cc, l2_cc
        );

        // Tight tolerance: exact contract on one-hot in F32
        assert!(
            linf_cm <= 1e-6,
            "candle vs manual mismatch at i={}: L_inf={}",
            i,
            linf_cm
        );
        assert!(
            linf_cc <= 1e-6,
            "orientation mismatch at i={}: L_inf(candle,column)={}",
            i,
            linf_cc
        );
    }
    ...
    assert!(
        idx_parsed.weight_map.contains_key("lm_head.weight"),
        "missing lm_head.weight in index"
    );
    assert!(
        idx_parsed
            .weight_map
            .contains_key("model.embed_tokens.weight"),
        "missing model.embed_tokens.weight in index"
    );
    Ok(())
}
...
#[test]
fn gpt_oss_lm_head_contract_cuda_skipped() {
