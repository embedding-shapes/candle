...
#[test]
fn gpt_oss_yarn_place_softmax() -> Result<()> {
    // Mode selection must be set by the harness: CANDLE_YARN_MODE=softmax
    let mode = std::env::var("CANDLE_YARN_MODE").unwrap_or_else(|_| "<unset>".to_string());
    eprintln!("CANDLE_YARN_MODE={} (expected 'softmax')", mode);
    ...
    eprintln!("-- Mode=B (softmax) top-10 next-token candidates --");
    for &i in &idx[..top] {
        let id = i as u32;
    ...
    // Assertions for Mode B: expect NOT to produce <|channel|> as top-1 under incorrect scaling.
    let argmax = idx[0];
    assert_ne!(argmax, channel_id, "Mode B (softmax) should not have <|channel|> as top-1");
