...
fn t42_router_linear_parity() -> Result<()> {
    ...
    assert!(l0.pp("mlp").contains_tensor("router.weight"), "missing mlp.router.weight");
    assert!(l0.pp("mlp").contains_tensor("router.bias"), "missing mlp.router.bias (bias required)");
    ...
    let x_f32: Vec<f32> = (0..hidden).map(|i| (((i as i32 % 13) - 6) as f32) / 10.0).collect();
    ...
    let linf = y_lin_v.iter().zip(&y_ref_v).map(|(a,b)| (a-b).abs()).fold(0f32, f32::max);
    let l2 = y_lin_v.iter().zip(&y_ref_v).map(|(a,b)| (a-b)*(a-b)).sum::<f32>().sqrt();
    ...
    assert!(linf < 5e-7 && l2 < 5e-6, "router linear mismatch: Linf={linf} L2={l2}");
    ...
    let linf_b = diff_v.iter().zip(&b_v).map(|(a,b)| (a-b).abs()).fold(0f32, f32::max);
    println!("bias-check Linf={:.3e}", linf_b);
    // In BF16/F16 we allow a tiny rounding difference for the bias broadcast path.
    assert!(linf_b < 3e-3, "router bias not applied correctly: Linf={linf_b}");
