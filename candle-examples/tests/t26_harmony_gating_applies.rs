...
fn t26_harmony_gating_masks_disallowed_specials() -> Result<()> {
    ...
    let to_mask: Vec<u32> = global_forbid.iter().copied().filter(|id| !allowed_ids.contains(id)).collect();
    ...
    let _ = sampler.sample_f(&logits, |prs: &mut [f32]| {
        for id in &to_mask {
            let i = *id as usize; if i < prs.len() { prs[i] = 0.0; }
        }
    ...
    for id in &to_mask {
        assert_eq!(prs[*id as usize], 0.0, "disallowed special id {} must be masked to 0", id);
    }
