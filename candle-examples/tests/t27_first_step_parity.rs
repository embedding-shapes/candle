...
fn t27_first_step_logit_parity() -> Result<()> {
    ...
    let input_ids: Vec<u32> =
        render_then_encode(&snapshot, &[user], true /* add_generation_prompt */)
...
"#,
    ...
    eprintln!(
        "[parity] candle top-1={} p_channel={:.6}",
        argmax, p_channel
    );
    eprintln!(
        "[parity] hf top-1={} p_channel={:.6}",
        py_res.top1_id, py_res.p_channel
    );
