...
#[command(
    author,
    version,
    about = "Microcheck: final hidden (post-norm, pre-lm_head) parity"
)]
struct Args {
...
fn main() -> Result<()> {
    ...
    println!(
        "last-hidden-post-norm: len={} first8={:?} mean={:.6} std={:.6}",
        v.len(),
        first8,
        mean,
        std
    ...
    if let Some(py_path) = args.compare_layers_npy.as_ref() {
        ...
        let (rb, cb) = (ra, ca);
        if ra != rb || ca != cb {
            anyhow::bail!("shape mismatch rust={}x{} python={}x{}", ra, ca, rb, cb);
        }
    ...
    if args.compare_python {
        ...
        let py_post_norm: Vec<f32> = vjson["post_norm_last"]
            .as_array()
        ...
        println!(
            "compare-python: post_norm_last Linf={:.6e} L2={:.6e}",
            linf, l2
        );
    }
    ...
    if args.debug_l0 || args.compare_python_l0 || args.compare_python_l0_attn {
        ...
        println!(
            "env CANDLE_YARN_MODE: {}",
            std::env::var("CANDLE_YARN_MODE").unwrap_or_else(|_| "<unset>".into())
        );
        ...
        println!(
            "v_pre: dtype=f32 shape=({}, {})",
            v_pre.dims2()?.0,
            v_pre.dims2()?.1
        );
        println!(
            "q_rope: dtype=f32 shape=({}, {})",
            q_rope.dims2()?.0,
            q_rope.dims2()?.1
        );
        println!(
            "k_rope: dtype=f32 shape=({}, {})",
            k_rope.dims2()?.0,
            k_rope.dims2()?.1
        );
        ...
        fn head_stats(tag: &str, m: &Tensor) -> anyhow::Result<()> {
            ...
            let std = if cnt == 0 {
                ...
                (var / cnt as f64).sqrt() as f32
            };
            println!(
                "{}: heads={} dim={} head0.first8={:?} mean={:.6} std={:.6}",
                tag, h, d, first8, mean, std
            );
            Ok(())
        }
        ...
        if args.compare_python_l0 {
            ...
            fn vec2_from_json(
                ...
                if arr.len() != rows * cols {
                    anyhow::bail!("size mismatch: got {} wanted {}", arr.len(), rows * cols);
                }
            ...
            println!(
                "compare-python-l0: q_pre  Linf={:.6e} L2={:.6e}",
                linf_qp, l2_qp
            );
            println!(
                "compare-python-l0: k_pre  Linf={:.6e} L2={:.6e}",
                linf_kp, l2_kp
            );
            println!(
                "compare-python-l0: v_pre  Linf={:.6e} L2={:.6e}",
                linf_vp, l2_vp
            );
            println!(
                "compare-python-l0: q_rope Linf={:.6e} L2={:.6e}",
                linf_qr, l2_qr
            );
            println!(
                "compare-python-l0: k_rope Linf={:.6e} L2={:.6e}",
                linf_kr, l2_kr
            );
            ...
            let py_attn_pre = vjson["attn_pre_last"]
                .as_array()
                .unwrap()
                .iter()
                .map(|x| x.as_f64().unwrap_or(0.0) as f32)
                .collect::<Vec<f32>>();
            let py_attn_post = vjson["attn_post_last"]
                .as_array()
                .unwrap()
                .iter()
                .map(|x| x.as_f64().unwrap_or(0.0) as f32)
            ...
            let mut linf_a = 0f32;
            let mut l2_a = 0f64;
            for (a, b) in y_pre_r.iter().zip(py_attn_pre.iter()) {
                let d = a - b;
                if d.abs() > linf_a {
                    linf_a = d.abs();
                }
                l2_a += (d as f64) * (d as f64);
            }
            let mut linf_b = 0f32;
            let mut l2_b = 0f64;
            for (a, b) in y_post_r.iter().zip(py_attn_post.iter()) {
                let d = a - b;
                if d.abs() > linf_b {
                    linf_b = d.abs();
                }
                l2_b += (d as f64) * (d as f64);
            }
            println!(
                "compare-python-l0: attn_pre  Linf={:.6e} L2={:.6e}",
                linf_a,
                l2_a.sqrt()
            );
            println!(
                "compare-python-l0: attn_post Linf={:.6e} L2={:.6e}",
                linf_b,
                l2_b.sqrt()
            );
            ...
            if args.compare_python_l0_attn {
                let (logits_last, weights_last) =
                    model.debug_l0_attn_last_token_logits_weights(&t, 0, 2)?; // (h<=2, klen)
                ...
                fn vec_from_json(opt: Option<&serde_json::Value>) -> Option<Vec<f32>> {
                    opt.and_then(|v| v.as_array()).map(|arr| {
                        arr.iter()
                            .map(|x| x.as_f64().unwrap_or(0.0) as f32)
                            .collect::<Vec<f32>>()
                    })
                ...
                if let Some(py_logits_h0) = vec_from_json(vjson.get("attn_logits_last_h0")) {
                    let mut linf = 0f32;
                    let mut l2 = 0f64;
                    for (a, b) in logits_r[0].iter().zip(py_logits_h0.iter()) {
                        let d = a - b;
                        if d.abs() > linf {
                            linf = d.abs();
                        }
                    ...
                    println!(
                        "compare-python-l0: logits_last h0 Linf={:.6e} L2={:.6e}",
                        linf,
                        l2.sqrt()
                    );
                ...
                if let Some(py_scores_h0) = vec_from_json(vjson.get("attn_scores_last_h0")) {
                    let mut linf = 0f32;
                    let mut l2 = 0f64;
                    for (a, b) in weights_r[0].iter().zip(py_scores_h0.iter()) {
                        let d = a - b;
                        if d.abs() > linf {
                            linf = d.abs();
                        }
                    ...
                    println!(
                        "compare-python-l0: scores_last h0 Linf={:.6e} L2={:.6e}",
                        linf,
                        l2.sqrt()
                    );
                ...
                if logits_r.len() > 1 {
                    if let Some(py_logits_h1) = vec_from_json(vjson.get("attn_logits_last_h1")) {
                        if let Some(py_logits_h1) = Some(py_logits_h1) {
                            let mut linf = 0f32;
                            let mut l2 = 0f64;
                            for (a, b) in logits_r[1].iter().zip(py_logits_h1.iter()) {
                                let d = a - b;
                                if d.abs() > linf {
                                    linf = d.abs();
                                }
                            ...
                            println!(
                                "compare-python-l0: logits_last h1 Linf={:.6e} L2={:.6e}",
                                linf,
                                l2.sqrt()
                            );
                        }
                    ...
                    if let Some(py_scores_h1) = vec_from_json(vjson.get("attn_scores_last_h1")) {
                        if let Some(py_scores_h1) = Some(py_scores_h1) {
                            let mut linf = 0f32;
                            let mut l2 = 0f64;
                            for (a, b) in weights_r[1].iter().zip(py_scores_h1.iter()) {
                                let d = a - b;
                                if d.abs() > linf {
                                    linf = d.abs();
                                }
                            ...
                            println!(
                                "compare-python-l0: scores_last h1 Linf={:.6e} L2={:.6e}",
                                linf,
                                l2.sqrt()
                            );
                        }
